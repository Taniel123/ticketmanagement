    ...
    <div class="max-w-md w-full relative bg-white p-8 rounded-xl shadow-xl overflow-hidden">
        ...
        <div class="relative">
            <div class="text-center mb-6">
                ...
                <h2 class="mt-4 text-3xl font-bold text-gray-800">Verify Your Email</h2>
                <p class="mt-2 text-base text-gray-600">
                    We sent a verification link to your email. If you didn't get it, request a new one.
                </p>
            </div>
            ...
            <form action="{{ route('verification.resend') }}" method="POST" class="space-y-6">
                @csrf
                <button type="submit" class="w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 transition">
            ...
            <div class="mt-8 pt-6 border-t border-gray-200">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium transition-colors w-full text-center">
                        Log Out
                    </button>
                </form>
            </div>
